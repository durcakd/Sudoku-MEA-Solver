...
MEA::~MEA(){
	std::list< Agent_sudoku * >::iterator  itAgent;
...
int MEA:: optimize(){
	...
	while(counterTrial < maxFitTrias){
		...
		if(NULL != solution){
			...
			if(! testMode){
				Agent_sudoku::printState(solution);
			}
			return counterTrial;
		}
...
int MEA:: initialization(){
	...
	if(NULL == (fixedState = new int[NN_NN])){
		CLogger::Instance()->write("\nError: malloc - fixedState in initialization()");
		return 1;
	}
	
	if(NULL == (file = fopen(sudokuFile, "r"))){
		outstr.Format("Error: cannot open file %s", sudokuFile); 
		CLogger::Instance()->write(outstr);
		return 1;
	}
	...
	if(EOF == fclose(file)){
		outstr.Format("Error: cannot close file %s", sudokuFile); CLogger::Instance()->write(outstr);
	}
	if(i != NN_NN){
		CLogger::Instance()->write("Error: load Sudoku frm file (no exactly 81 numbers)");
		return 1;
	}
	//===================================================
	...
	if(NULL == (fixedLists = new int[NN_NN + NN])){
		CLogger::Instance()->write("Error: malloc - fixedLists in initialization()");
		return 1;
	}
	...
	if(NULL == (tabuList = new int[NN_NN*NN])){
		CLogger::Instance()->write("Error: malloc - tabuLists in initialization()");
		return 1;
	}
		// make tabu cube2
...
int MEA:: generateAgents(){
	...
	for(i = 0; i < parNumAgents;  ++i)
		agents.push_back(new Agent_sudoku(parStartLifePoints, fixedState, fixedLists, parMaxTrials,	parMutProbability, tabuList));
...
int MEA:: decLifePointsAndEliminate(){
	std::list< Agent_sudoku * >::iterator  itAgent, auxIt;
	CString outstr;
...
int MEA:: birthNewAgent(){
	...
	if(NULL != randomState.state)
		agents.push_back(new Agent_sudoku(parStartLifePoints, fixedState, fixedLists, randomState.state, randomState.fitness, parMaxTrials,	parMutProbability,tabuList));
	else
		agents.push_back(new Agent_sudoku(parStartLifePoints, fixedState, fixedLists, parMaxTrials,	parMutProbability, tabuList));
...
void MEA:: printAgents(){
	...
	outstr.Format("Agents count = %d", agents.size()); CLogger::Instance()->write(outstr);
	
	for(itAgent = agents.begin(); itAgent != agents.end();  ++itAgent){
		outstr.Format("  agent %3d:  life %3d,  F %3d,  M %3d,  CS %p",
			(*itAgent)->getName(),
			(*itAgent)->getLifePoints(), 
			(*itAgent)->getCurrentFitness(), 
			(*itAgent)->getBestMilestoneFitness(),
			(*itAgent)->getCurrentState() );
...
void MEA:: printBestMeanFitAndAgentsSize(){
	...
	if(agents.empty()){
		CLogger::Instance()->write("Warning  : agents.size() == 0,  (printBestMeanFitAndAgentsSize())");
		return;
	}
	...
	outstr.Format("%3d ; %3d ; %3d ; %3d ; %4d;  %3d;  %3d; %3d", bestFit, meanFit, bestElitFit, meanElitFit , agents.size(), eliteList.getBestFitness(), eliteList.getMeanFitness(), eliteList.getNumberOfEliteStates()  );
...
void MEA:: testEliteList(){
	...
	CLogger::Instance()->write("TEST Elite List");
	...
	std::list< Agent_sudoku * >::iterator   auxIt;
	int i;
	CLogger::Instance()->write("TEST decAndEliminate()  LIFE POINTS:");
	i = 0;
	for(itAgent = agents.begin(); itAgent != agents.end(); ++itAgent){
		//if(i %3 == 0)
		if(i %3 == 0 || i %3 == 2)
	...
	for(itAgent = agents.begin(); itAgent != agents.end();  ){
		auxIt = itAgent;
		++itAgent;
		//printf("\nM(%d)***%3d  ",(*auxIt)->getName(),(*auxIt)->getLifePoints() );
		outstr.Format("M(%d)***%3d  ",(*auxIt)->getName(),(*auxIt)->getLifePoints() );
		
		if(0 >= (*auxIt)->decLifePoints()){
			agents.erase(auxIt);
			//printf("-> agent kaput");
			outstr.Format(outstr + "-> agent kaput");
